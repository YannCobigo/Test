...
namespace MAC
{
  ...
  //
  template < int D >
  MAC::Subject< D >::Subject( const int Pidn,
			      const int Group):
  ...
  //
  template < int D > void
    MAC::Subject< D >::build_design_matrices( const double Age_mean )
  {
    try
      {
	//
  ...
  //
  template < int D > void
    MAC::Subject< D >::add_tp( const int                  Age,
	...
	if ( age_covariates_.find( Age ) == age_covariates_.end() )
	  {
		...
		throw MAC::BmleException( __FILE__, __LINE__,
					       mess.c_str(),
					       ITK_LOCATION );
	      }
	    //
  ...
  //
  template < int D > void
    MAC::Subject< D >::create_theta_images()
    {
