...
$\delta_{l_{u}}^{(s)}$ is the estimation of the error at the feture map $s$, at the position $(x,y,z)_{l_{u}}$. Using equation~(\ref{eq:convolution_tot_vox}) we can etimate the second part of the Eq.~(\ref{eq:conv_backward_weights}). For all activatios of the layer $l_{u}$, we have:
