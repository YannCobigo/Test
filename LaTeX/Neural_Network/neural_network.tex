...
\begin{equation*}
    (Im*Ker)_{xyz} =  \sum_{i=1}^{I}\sum_{j=1}^{J}\sum_{k=1}^{K}\sum_{m=1}^{M}Im_{x+i,y+j,z+k}Ker_{-i,-j,-k,-m} - b
...
\begin{equation*}
    v_{x,y,z,m}^{l} =  \sum_{i=1}^{I}\sum_{j=1}^{J}\sum_{k=1}^{K}\omega_{-i,-j,-k,-m}^{l}o_{x+i,y+j,z+k,m}^{l-1} - b^{l}
  \label{eq:convolution_tot} 
\end{equation*}

where $v$ is the convolved voxel, $l$ is any layer between $l = 1$ and $l = L$, $\omega$ is the weight at the layer $l$ (Fig.~\ref{fig:Kernel}), $b$ is the bias at the level $l$, $o$ is the output at the level $l-1$ ($o_{x,y,z,m}^{l} = f(x_{x,y,z,m}^{l})$ where $f$ is an activation function)



\begin{figure}[htbp]
   \begin{center}
      \includegraphics[scale=0.2, angle=0]{images/GvsBA.jpg}
   \end{center}
...
\begin{figure}[htbp]
   \begin{center}
      \includegraphics[scale=0.2, angle=0]{images/densely_connected_nn.png}
   \end{center}
