...
\begin{equation}
    a_{xyz;l_{u}}^{(\theta)} =  \sum_{i=1,j=1,k=1}^{I \times J \times K}\omega_{ijk;l_{u}l_{u-1}}^{(\theta)}z_{x+i,y+j,z+k;l_{u-1}}^{(\theta)} - b_{l_{k}}^{(\theta)}
  \label{eq:convolution_tot_vox} 
\end{equation}

where $a$ is the convolved activation voxel in the feature map, $l_{u}$ is any layer, $\omega$ is the weight at the layer $l_{u}$ (Fig.~\ref{fig:Kernel}), $b$ is the bias at the level $l_{u}$, $z$ is the output at the level $l_{u-1}$ ($z_{xyz;l_{u}}^{(\theta)} = f(a_{xyz;l_{u}}^{(\theta)})$ where $f$ is an activation function). $theta$ is the set of parameters describing the the modality and the set of kernels associated to the different feature maps.
...
\begin{equation}
    \frac{\partial E_{i}}{\partial \omega_{uvw;l_{u}l_{u-1}}^{(\theta)}} = \sum_{xyz}^{X \times Y \times Z} \frac{\partial E_{i}}{\partial a_{xyz;l_{u}}^{(\theta)}}\frac{\partial a_{xyz;l_{u}}^{(\theta)}}{\partial \omega_{uvw;l_{u}l_{u-1}}^{(\theta)}}  \\
...
\begin{equation*}
    \left( \frac{\partial a_{xyz;l_{u}}}{\partial \omega_{uvw;l_{u}l_{u-1}}} \right)^{(\theta)} = z_{x'+u,y'+v,z'+w;l_{u-1}}^{(\theta)}
\end{equation*}

Where $(x',y',z')$ are in the receiptive field of $(x,y,z)$. The second part of the equation can be solved in the following way:
...
\begin{equation}
  \begin{split}
    ...
    =& \sum_{x''y''z''}^{X \times Y \times Z} \delta_{l_{u+1}}^{(\theta)}(x'',y'',z'') \times \frac{\partial }{\partial a_{xyz;l_{u}}^{(\theta)}} \sum_{abc}\omega_{abc;l_{u+1}l_{u}}^{(\theta)}f(a_{\tilde{x}+a,\tilde{y}+b,\tilde{z}+c;l_{u}}^{(\theta)})\\
    =& \sum_{x''y''z''}^{X \times Y \times Z} \delta_{l_{u+1}}^{(\theta)}(x'',y'',z'') \times \sum_{abc}\omega_{abc;l_{u+1}l_{u}}^{(\theta)} \delta(x-(\tilde{x}+a),y-(\tilde{y}+b),z-(\tilde{z}+c)) f'(a_{\tilde{x}+a,\tilde{y}+b,\tilde{z}+c;l_{u}}^{(\theta)})\\
  \end{split}
  \label{} 
\end{equation}
\end{strip}

Where $\{(x'',y'',z'')_{l_{k}}\}$ is a set of activation sites on the layer $l_{u+1}$ having $(x,y,z)_{l_{k}}$ in its receiving field. We keep the $\delta(x-(\tilde{x}+a),y-(\tilde{y}+b),z-(\tilde{z}+c))$ Dirac distribution in the sum, since we will select the $(a,b,c)$ to get an input output correspondance. In our case, we should have a strict correspondance between $(x',y',z')_{l_{k}}$ and $(x,y,z)_{l_{k-1}}$.
