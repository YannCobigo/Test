...
namespace MAC_nip
{
  ...
  private:
    ...
    // Save Matrices spectra
    std::shared_ptr< Spectra > matrices_spetrum_;
    //
    ...
    // grid size
    int grid_size_im_{1000}; //1000
    int grid_size_ev_{100}; //100
    //
  ...
  template< int K, int CPU >
    Nip_PMD_cross_validation<K,CPU>::Nip_PMD_cross_validation( std::shared_ptr< const Eigen::MatrixXd > Images_matrix,
							       ...
							       std::shared_ptr< Spectra >               Matrices_spetrum ):
    images_matrix_{Images_matrix}, ev_matrix_{Ev_matrix}, matrices_spetrum_{Matrices_spetrum}, image_features_{Images_matrix->cols()}, ev_features_{Ev_matrix->cols()}
  {
    try
      {
	//
  ...
  template< int K, int CPU > void
    Nip_PMD_cross_validation<K,CPU>::validation()
    {
      try
	{
	  //
	  ...
	  //
	  // Create the spectrum
	  std::size_t K_cca = matrices_spetrum_->size();
	  MAC_nip::NipPMD pmd_cca;
	  ...
	  pmd_cca.K_factors( images_norm.transpose() * ev_norm,
			     *matrices_spetrum_.get(), L1, L1, true );
