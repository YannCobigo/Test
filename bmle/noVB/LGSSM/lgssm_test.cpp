  ...
  for ( int i = 0 ; i < N ; i++ )
    {
      ...
      //
      int
	Ti     = 1000, //TP( generator ),
	age    = AGE( generator ),
	status = SICK( generator );
      //
      LGSSM_intensity[i].resize(Ti);
      LGSSM_age[i].resize(Ti);
      //
      std::cout
	<< "Subject " << i
      ...
      if ( status == 0 )
	{
	  ...
	  for ( int t = 0 ; t < Ti ; t++ )
	      ...
	      if ( Dim == 4 )
		{
		  ...
		  Eigen::Matrix< double, Dim, 1 > mu = NeuroBayes::gaussian_multivariate< Dim >( ( t == 0 ? mu_cn : LGSSM_intensity[i][t-1] ),
												 sigma_cn );
		  //
		  ...
		  std::cout << "\t tp "<<t<<" mu = \n" << mu << std::endl;
		  //
		  LGSSM_age[i][t]       << static_cast< double >( age++ );
		  LGSSM_intensity[i][t] = mu;
		}
	    }	  
	}
      //
...
//    LGSSM_intensity[3].resize(2);
//  LGSSM_intensity[3][0] << 0.222687,0.66818,0.105976,0.00315745;
//  LGSSM_intensity[3][1] << 0.00395141,0.724707,0.271204,0.000137096;
////  LGSSM_intensity[7].resize(1);
////  LGSSM_intensity[7][0] << 0.295926,0.574033,0.126111,0.00392977;
//  LGSSM_intensity[4].resize(3);
//  LGSSM_intensity[4][0] << 0.251824,0.635765,0.106653,0.00575759;
//  LGSSM_intensity[4][1] << 0.143318,0.789453,0.0593005,0.00792879;
//  LGSSM_intensity[4][2] << 0.0167064,0.921917,0.00813137,0.0532457;
//  LGSSM_intensity[5].resize(2);
//  LGSSM_intensity[5][0] << 0.237294,0.63782,0.121552,0.00333464;
//  LGSSM_intensity[5][1] << 0.455978,0.456958,0.0828703,0.0041939;
//  LGSSM_intensity[6].resize(2);
//  LGSSM_intensity[6][0] << 0.0403983,0.129326,0.017083,0.813193;
//  LGSSM_intensity[6][1] << 0.12638,0.330314,0.10596,0.437346;
